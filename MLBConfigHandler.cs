...
{
    public class MLBConfigHandler
    {
        private readonly LineshouseContext _lineshouseContext;
        private readonly DgsContext _dgsContext;

        public MLBConfigHandler(LineshouseContext context, DgsContext dgsContext)
        {
        ...
        public void Handle()
        {
            ...
            var leaguesConfig = this._dgsContext.Config.Include(x => x.League).Where(x => x.League.SportId == 3).Where(x => (new string[] { "MLB" }.Contains(x.League.Description))).ToList();
