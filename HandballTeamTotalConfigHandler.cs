...
{
    public class HandballTeamTotalConfigHandler
    {
        private readonly LineshouseContext _lineshouseContext;
        private readonly DgsContext _dgsContext;

        public HandballTeamTotalConfigHandler(LineshouseContext context, DgsContext dgsContext)
        {
        ...
        public void Handle()
        {
            ...
            var leaguesConfig = this._dgsContext.Config.Include(x => x.League).Where(x => x.League.SportId == 18).ToList();
            ...
            foreach (var config in leaguesConfig)
            {
                ...
                var oldPropConfig = data.FirstOrDefault(x => x.Key == "0,1");
